	...
	public void run() {
		...
		while (true) {
			...
			synchronized (lock) {
				...
				double WR=2.05; //wheel radius
				double WB=15.3;	//width of robot
				nowTachoL = Motor.A.getTachoCount(); // get tacho counts 
				nowTachoR = Motor.C.getTachoCount();
				distL = Math.PI*WR*(nowTachoL-lastTachoL)/180; //get distance travelled by each wheel at each tacho count
				distR = Math.PI*WR*(nowTachoR-lastTachoR)/180;
				lastTachoL=nowTachoL;
				lastTachoR=nowTachoR;
				deltaD = 0.5*(distL+distR); //average of the error difference travelled by each wheel
				deltaT = (distL-distR)/WB;	//theta deviation
				theta += deltaT;
